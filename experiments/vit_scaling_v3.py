...
# -*- coding: utf-8 -*-
"""
ESPERIMENTO: Scaling ViT su CIFAR-100 — v3 con regolarizzazione calibrata per scala
======================================================================================
Rispetto a v2, calibra la forza della regolarizzazione rispetto alla capacità
del modello. In v2 la regolarizzazione uniforme causava sotto-adattamento alla
scala Tiny (-4.99 pp vs v1). v3 riduce la regolarizzazione per Tiny:

  Tiny:  RandAugment mag 5 (era 9), CutMix/Mixup prob 0.5 (era 1.0), DropPath 0.05 (era 0.1)
  Small: invariato rispetto a v2
  Base:  invariato rispetto a v2

Genera inoltre plot per il paper con confronto v1/v2/v3.

Uso:
    # Lancia tutto (tutte le scale, NOVA vs GELU su 2 GPU):
    python vit_scaling_v3.py

    # Singolo esperimento:
    python vit_scaling_v3.py --scale tiny --activation nova --gpu 0

    # Solo generazione plot (dopo aver completato gli esperimenti):
    python vit_scaling_v3.py --plot-only
"""
...
SCALING_CONFIGS = {
    "tiny": {
        ...
        "dropout": 0.1,
        "weight_decay": 0.05,
        # v3: regolarizzazione ridotta per Tiny (3.2M params)
        # v2 causava sotto-adattamento con dropout_path=0.1, RandAug mag=9, CutMix prob=1.0
        "drop_path_rate": 0.05,
        "cutmix_alpha": 1.0,
        "mixup_alpha": 0.8,
        "cutmix_mixup_prob": 0.5,  # era 1.0 — applicato solo al 50% dei batch
        "switch_prob": 0.5,
        "randaug_num_ops": 2,
        "randaug_magnitude": 5,    # era 9 — ridotto per preservare capacità
    },
    "small": {
        "embed_dim": 384,
        "num_heads": 6,
...
def run_experiment(scale: str, activation: str, gpu: int) -> None:
    ...
    log_data = {
        ...
        "dataset": DATASET,
        "modello": model_desc,
        "obiettivo": (f"Scaling study v3 (anti-overfit): {scale} ViT, "
    ...
    for epoch in range(epochs):
        ...
        print(f"[{scale.upper()}/{activation.upper()}/v3] "
    ...
    print(f"\n{'='*60}")
    print(f"  RISULTATI v3 — {scale.upper()} / {activation.upper()}")
    ...
    print(f"[{scale.upper()}/{activation.upper()}/v3] VRAM liberata.")
...
def generate_plots():
    """Genera plot per il paper con confronto v1/v2/v3."""
    import matplotlib
    matplotlib.use('Agg')
    ...
    for scale in scales:
        for act in activations:
            ...
            v3 = load_latest(f"vit_scaling_v3_{scale}_{act}_*.json")
            if v1:
                v1_data[(scale, act)] = v1
            if v2:
                v2_data[(scale, act)] = v2
            if v3:
                v3_data[(scale, act)] = v3

    if not v3_data:
        print("[PLOT] Nessun risultato v3 trovato. Esegui prima gli esperimenti.")
        return
    ...
    # ==========================================================
    # PLOT 1: Training curves v3 per scala (val_acc, train/val loss)
    # ==========================================================
    fig, axes = plt.subplots(2, 3, figsize=(15, 8))
    fig.suptitle("ViT Scaling v3 — Training Curves (CIFAR-100, 100 epoche)",
    ...
    plt.tight_layout()
    path = os.path.join(results_dir, "plot_v3_training_curves.png")
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"[PLOT] Salvato: {path}")

    # ==========================================================
    # PLOT 2: Scaling curve — best val acc vs params (v1 vs v2 vs v3)
    # ==========================================================
    fig, ax = plt.subplots(figsize=(9, 5.5))
    ax.set_title("Scaling Curve: Best Val Accuracy vs Model Size (v1/v2/v3)",
    ...
    plt.tight_layout()
    path = os.path.join(results_dir, "plot_v3_scaling_curve.png")
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"[PLOT] Salvato: {path}")

    # ==========================================================
    # PLOT 3: Evoluzione di β (NOVA) v3 per le 3 scale
    # ==========================================================
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.set_title("Evoluzione del parametro β (NOVA) — v3",
                 fontsize=13, fontweight='bold')
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    path = os.path.join(results_dir, "plot_v3_beta_evolution.png")
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"[PLOT] Salvato: {path}")

    # ==========================================================
    # PLOT 4: Confronto NOVA v1 vs v2 vs v3 per scala
    # ==========================================================
    fig, axes = plt.subplots(1, 3, figsize=(15, 4.5))
    fig.suptitle("NOVA: Val Accuracy v1 / v2 / v3 per scala",
                 fontsize=13, fontweight='bold')
    ...
    for col, scale in enumerate(scales):
        ax = axes[col]
        for version, data_dict, ls, alpha in version_styles:
            key = (scale, "nova")
            if key not in data_dict:
                continue
            data = data_dict[key]
            ...
            ax.plot(epochs_list, val_accs, linestyle=ls,
                    color=colors["nova"], alpha=alpha, linewidth=1.8,
    ...
    plt.tight_layout()
    path = os.path.join(results_dir, "plot_v3_nova_comparison.png")
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"[PLOT] Salvato: {path}")

    # ==========================================================
    # PLOT 5: Bar chart NOVA best val acc per scala (v1/v2/v3)
    # ==========================================================
    fig, ax = plt.subplots(figsize=(9, 5))
    ax.set_title("NOVA Best Val Accuracy: v1 vs v2 vs v3",
                 fontsize=13, fontweight='bold')

    x = np.arange(len(scales))
    width = 0.25
    version_data = [("v1", v1_data), ("v2", v2_data), ("v3", v3_data)]
    bar_colors_v = {"v1": "#F4A261", "v2": "#E76F51", "v3": "#E63946"}

    for i, (version, data_dict) in enumerate(version_data):
        accs = []
        for scale in scales:
            key = (scale, "nova")
            if key in data_dict:
                accs.append(data_dict[key]["metriche"]["best_val_acc"])
            else:
                accs.append(0)
        bars = ax.bar(x + (i - 1) * width, accs, width,
                      label=f"NOVA ({version})", color=bar_colors_v[version],
    ...
    plt.tight_layout()
    path = os.path.join(results_dir, "plot_v3_nova_bar_comparison.png")
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"[PLOT] Salvato: {path}")
...
def launch_all(scales=None, activations=None):
    ...
    print(f"\n{'='*60}")
    print("  SCALING STUDY v3 COMPLETATO")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ViT Scaling v3 (calibrated regularization) CIFAR-100: NOVA vs GELU")
